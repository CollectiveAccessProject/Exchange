          ...
          if (value['media'])
            ...
            value['media'].split('|').each do |u|
              if ((key = /representation:([\d]+)/.match(u)))
                ...
                if (!m)
                  m = MediaFile.new(title: key[0] + ":" + key[1] + ":" + i.to_s, caption: '', copyright_notes: '')
                  m.save
                end

                m.set_sourceable_media({collectiveaccess_link: { original_link: u}})
                m.update({title: key[0] + ":" + key[1] + ":" + i.to_s, caption: value['caption_text'], access: 1, copyright_notes:i.to_s, resource_id: r.id})
